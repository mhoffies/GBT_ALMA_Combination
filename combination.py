...
for f in myfiles:
     ...
     print('------------')
     for key in mykeys:
         q = imhead('flagged_image_r_2.0.image',mode='get',hdkey=key)
         print(str(key)+' : '+str(q))
'''
...
# trim extra channels off of GBT data

imsubimage(imagename='GBT.image.regrid.ro.subim',
           outfile='GBT.image.regrid.ro.subim.trim',
           chans='89~288')


# Multiply the flux by the GBT image to get the same response

immath(imagename=['GBT.image.regrid.ro.subim.trim',
                  'pb.subim'],
       expr='IM0*IM1',
...
imsubimage(imagename='GBT.smooth.regrid.ro',
           outfile='GBT.smooth.regrid.ro.trim',
           chans='89~288')


# Trim cube to correct size 

imsubimage(imagename='GBT.smooth.regrid.ro.trim',
           outfile='GBT.smooth.regrid.ro.trim.subim',
           box='143,143,366,366')


# Multiply by primary beam response 

immath(imagename=['GBT.smooth.regrid.ro.trim.subim',
                  'pb.subim'],
       expr='IM0*IM1',
